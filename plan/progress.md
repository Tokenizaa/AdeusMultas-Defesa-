...
| 2026-09-15T15:58:33Z | G1-01 — Schema de banco e RLS | **PASS — evidência Supabase em produção** — `supabase_migrations.schema_migrations` contém migrações aplicadas; `public.cases` e o atual `public.user_profiles` estão com RLS habilitado; `public.cases` possui políticas de acesso e `public.user_profiles` possui políticas de perfil/admin. O critério histórico `profiles` foi reconciliado para `user_profiles`, nome canônico atual. Nenhuma alteração de schema foi necessária. |
