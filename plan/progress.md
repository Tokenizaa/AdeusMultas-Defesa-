...
| 2026-09-15T17:17:36Z | G2-02 — CRUD de casos com proteção IDOR | PASS — all case routes protected by authenticateToken; canAccessCase ensures user owns case or admin; POST creates case with userId from token; verified via src/server/routes/cases.ts. |
| 2026-09-15T17:20:00Z | G3-01 — Pipeline de geração de defesa com fallback determinístico | PASS — pipeline uses deterministic base, AI refinement with validation, fallback to deterministic on failure or unavailability; verified via src/core/ai/ai-orchestrator.ts and src/server/routes/cases.ts. |
| 2026-09-15T17:23:43Z | G3-02 — Gerenciador de documentos dinâmico por tipo de procedimento | PASS — engine selects template based on procedure type (7 supported procedures) and assembles document using blocks and arguments; verified via src/core/documents/document-assembly-engine.ts and src/core/rag/rag-pipeline.ts. |
| 2026-09-15T17:25:55Z | G4-01 — Integração Meta — adaptador unificado | PASS — Meta publisher adapter implements Publisher for Instagram/Facebook via Graph API; verified via cloudflare/publishers.ts and cloudflare/routes/marketing.ts. |
| 2026-09-15T17:30:45Z | G5-02 — Landing page e páginas públicas | PASS — landing page and public pages exist; responsive Tailwind; SEO via useSEO; accessibility via AccessibilityContext. |
| 2026-09-15T18:17:12Z | G5-01 — Gateway de pagamento unificado (PagBank + GGPIX) | PASS — GGPIX webhook route added; idempotent processing verified; gateway manager selects GGPIX in production. Commit: b67873f5f2e8dbf21bc7f63dee720ffb86c5b152 |
