...
describe('EnvelopeService', () => {
  ...
  describe('createEnvelopeFromCase', () => {
    it('should create envelope and upload PDF', async () => {
      ...
      const signers = [{ email: 'signer1@example.com', name: 'Signer One' }];
    ...
    it('should throw on PDF upload failure', async () => {
      ...
      await expect(service.createEnvelopeFromCase('case-123', pdfBuffer, signers)).rejects.toThrow(DocumensoError);
    });
    ...
    it('should create envelope with custom title', async () => {
      ...
      await service.createEnvelopeFromCase('case-123', pdfBuffer, signers, { title: 'Custom Title' });
    ...
    it('should create envelope with custom settings', async () => {
      ...
      await service.createEnvelopeFromCase('case-123', pdfBuffer, signers, {
        settings: { expiresInDays: 60, signingOrder: 'SEQUENTIAL' },
      });
  ...
  describe('sendEnvelope', () => {
    it('should send envelope successfully', async () => {
      const mockResponse = { id: 'env_abc123', status: 'PENDING', sentAt: '2024-01-01T00:00:00Z' };
  ...
  describe('getEnvelopeStatus', () => {
    it('should return envelope status', async () => {
      const mockResponse = { id: 'env_abc123', status: 'COMPLETED' };
      mockFetch.mockResolvedValueOnce({ ok: true, json: async () => mockResponse } as Response);
