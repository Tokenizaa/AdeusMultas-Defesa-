...
test.describe('Suíte E2E: Defesa de Cassação de CNH (PCDD)', () => {
  ...
  test.beforeAll(async () => {
    ...
    // Get all test users and cases for cassacao
    const allUsers = await testManager.getTestUsers();
    const allCases = await testManager.getTestCases();
    
    testUsers.push(...allUsers.filter(u => u.serviceType === 'cassacao'));
    testCases.push(...allCases.filter(c => c.serviceType === 'cassacao'));
    
    console.log(`[Cassação] Found ${testUsers.length} users and ${testCases.length} test cases`);
  });
  ...
  for (const testCase of testCases.slice(0, Math.max(4, testCases.length))) {
    test(`${testCase.serviceType} - ${testCase.scenario} (${testCase.procedureType}) - User ${testCase.id.slice(0, 8)}`, async ({ page }) => {
      ...
      await testManager.saveEvidence(`cassacao-${testCase.id}-result.json`, combinedResult);
