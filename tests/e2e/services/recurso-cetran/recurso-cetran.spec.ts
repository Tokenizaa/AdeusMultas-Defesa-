...
test.describe('Suíte E2E: Recurso Final ao CETRAN (2ª Instância)', () => {
  ...
  test.beforeAll(async () => {
    ...
    // Get all test users and cases for recurso-cetran
    const allUsers = await testManager.getTestUsers();
    const allCases = await testManager.getTestCases();
    
    testUsers.push(...allUsers.filter(u => u.serviceType === 'recurso_cetran'));
    testCases.push(...allCases.filter(c => c.serviceType === 'recurso_cetran'));
    
    console.log(`[Recurso CETRAN] Found ${testUsers.length} users and ${testCases.length} test cases`);
  });
  ...
  for (const testCase of testCases.slice(0, Math.max(4, testCases.length))) {
    test(`${testCase.serviceType} - ${testCase.scenario} (${testCase.procedureType}) - User ${testCase.id.slice(0, 8)}`, async ({ page }) => {
      ...
      await testManager.saveEvidence(`recurso-cetran-${testCase.id}-result.json`, combinedResult);
