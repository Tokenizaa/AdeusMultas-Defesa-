...
test.describe('Suíte E2E: Parecer Técnico de Consistência e Vícios Formais', () => {
  ...
  test.beforeAll(async () => {
    ...
    // Get all test users and cases for analise-tecnica
    const allUsers = await testManager.getTestUsers();
    const allCases = await testManager.getTestCases();
    
    testUsers.push(...allUsers.filter(u => u.serviceType === 'analise_tecnica'));
    testCases.push(...allCases.filter(c => c.serviceType === 'analise_tecnica'));
    
    console.log(`[Análise Técnica] Found ${testUsers.length} users and ${testCases.length} test cases`);
  });
  ...
  for (const testCase of testCases.slice(0, Math.max(4, testCases.length))) {
    test(`${testCase.serviceType} - ${testCase.scenario} (${testCase.procedureType}) - User ${testCase.id.slice(0, 8)}`, async ({ page }) => {
      ...
      await testManager.saveEvidence(`analise-tecnica-${testCase.id}-result.json`, combinedResult);
