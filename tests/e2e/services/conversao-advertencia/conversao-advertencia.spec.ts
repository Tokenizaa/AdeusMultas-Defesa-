...
test.describe('Suíte E2E: Requerimento de Conversão em Advertência (Art. 267)', () => {
  ...
  test.beforeAll(async () => {
    ...
    // Get all test users and cases for conversao-advertencia
    const allUsers = await testManager.getTestUsers();
    const allCases = await testManager.getTestCases();
    
    testUsers.push(...allUsers.filter(u => u.serviceType === 'conversao_advertencia'));
    testCases.push(...allCases.filter(c => c.serviceType === 'conversao_advertencia'));
    
    console.log(`[Conversão Advertência] Found ${testUsers.length} users and ${testCases.length} test cases`);
  });
  ...
  for (const testCase of testCases.slice(0, Math.max(4, testCases.length))) {
    test(`${testCase.serviceType} - ${testCase.scenario} (${testCase.procedureType}) - User ${testCase.id.slice(0, 8)}`, async ({ page }) => {
      ...
      await testManager.saveEvidence(`conversao-advertencia-${testCase.id}-result.json`, combinedResult);
