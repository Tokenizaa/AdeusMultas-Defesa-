...
adminRoutes.get('/admin/overview', async (c) => {
  ...
  return c.json({
    ...
    aiStatus: { primaryProvider: 'nvidia', fallbackProvider: '9router', nvidiaHealthy: false, nineRouterHealthy: false, fallbackRatePercent: 0, p95LatencyMs: 0 },
    integrationsHealth: { supabase: 'HEALTHY', pagbank: 'UNKNOWN', meta: 'UNKNOWN', ocr: 'UNKNOWN' },
  });
});
...
adminRoutes.get('/admin/users', async (c) => {
  ...
  const { data, error } = await supabase
    .from('user_profiles')
    .select('user_id,name,email,role,cpf,phone,cnh,city_state,avatar_url,created_at')
...
adminRoutes.put('/admin/users', async (c) => {
  ...
  if (!email || !['admin', 'citizen'].includes(role)) {
    return c.json({ ok: false, error: { code: 'VALIDATION_ERROR', message: 'email e role válidos são obrigatórios' } }, 400);
  }
  ...
  return c.json({
    ...
    user: {
      ...
      cityState: data.city_state || undefined,
...
adminRoutes.get('/admin/documents', async (c) => {
  ...
  return c.json({ documents: (data || []).map((d: any) => ({
    id: d.id, title: d.title, clientName: d.client_name, serviceType: d.service_type,
    status: d.status, hasDraft: Boolean(d.defense_draft_json), createdAt: d.created_at,
  })) });
});
