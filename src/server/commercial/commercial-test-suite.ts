...
export function runCommercialTestSuite(): CommercialTestSuiteSummary {
  ...
  try {
    ...
    results.push({
      ...
      expected: 'Preço ativo configurado para recurso_jari (catálogo comercial)',
  ...
  try {
    const validation = commercialService.validateCoupon('DEFESAI10', 100.0, 'recurso_jari');
  ...
  try {
    const validation = commercialService.validateCoupon('EXPIRADO2023', 100.0, 'recurso_jari');
