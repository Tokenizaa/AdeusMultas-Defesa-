...
const promo = (id: string, discountValue: number): PromotionCampaign => ({
  ...
  discountValue,
  applicableServices: ['defesa_previa', 'recurso_multa'],
...
const offer = new OfferService(
  new Map<string, ServicePricing>([
    ['price_defesa_previa', svc('defesa_previa', 8990, 4495)],
  ]),
...
// 1º documento → 2248
const r1 = offer.resolve({ serviceType: 'defesa_previa', documentCount: 0 });
...
const r2 = offer.resolve({ serviceType: 'defesa_previa', documentCount: 1 });
assert('doc2 final', r2.offer?.finalAmount ?? -1, 2248);
assert('doc2 numero', r2.offer?.documentNumber ?? -1, 2);

// 3º documento
const r3 = offer.resolve({ serviceType: 'defesa_previa', documentCount: 2 });
...
const r4 = offer.resolve({ serviceType: 'defesa_previa', documentCount: 3 });
