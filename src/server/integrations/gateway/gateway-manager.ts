...
function resolveActiveGatewayIdFromEnv(): GatewayId {
  ...
  if (configOverride && (configOverride === 'ggpixapi' || configOverride === 'pagbank' || configOverride === 'test')) {
    logger.info('payments', 'gateway_manager', 'resolve', 'Using ConfigService override for active gateway', { override: configOverride });
  ...
  // PagBank é uma integração oficial suportada também em produção.
  // Sem gateway explicitamente escolhido, preferimos PagBank quando configurado;
  // caso contrário usamos GGPIXAPI como fallback de produção.
  const paymentMode = (process.env.PAYMENT_MODE || 'sandbox').toLowerCase().trim();
  if (paymentMode === 'production') {
    if (pagbankAdapter.isConfigured()) return 'pagbank';
    return 'ggpixapi';
  }
  return 'pagbank';
}
