...
router.put('/onboarding-v2/draft', async (req, res) => {
  ...
  if (!payload) return res.status(400).json({ error: 'Payload canônico obrigatório.' });
  const current = CanonicalMapper.rowToDomain(row);
  const next = CanonicalMapper.onboardingPayloadToDomain(payload as CanonicalOnboardingPayload, current.id);
...
router.post('/onboarding-v2/cases/:id/analysis', async (req, res) => {
  ...
  const updated: CaseDomain = { ...domain, analysis, status: 'analisado', currentStage: 2, updatedAt: new Date().toISOString() };
  await databaseRows.set(domain.id, CanonicalMapper.domainToRow(updated));
  return res.json({ caseId: domain.id, status: 'completed', analysis });
});

router.get('/onboarding-v2/cases/:id/analysis', async (req, res) => {
  const row = await getAuthorizedCase(req, res, req.params.id); if (!row) return;
  const domain = CanonicalMapper.rowToDomain(row);
  if (!domain.analysis) return res.status(404).json({ caseId: domain.id, status: 'pending' });
  return res.json({ caseId: domain.id, status: 'completed', analysis: domain.analysis });
});
...
router.put('/onboarding-v2/cases/:id/qualification', async (req, res) => {
  ...
  const current = CanonicalMapper.rowToDomain(row);
  const updated: CaseDomain = { ...current, applicant: { ...applicant, applicantCpf: cpf }, clientName: applicant.applicantName, clientCpf: cpf, clientEmail: applicant.applicantEmail, clientPhone: applicant.applicantPhone, status: 'aguardando_pagamento', currentStage: 3, updatedAt: new Date().toISOString() };
