...
export function createOnboardingHttpApplication(client: OnboardingHttpClient, setClaimToken?: (token: string) => void): OnboardingApplication {
  ...
  return {
    ...
    async updateDraft(input) {
      const result = await client.request<{ case: CreateDraftResult['case'] }>('/api/onboarding-v2/draft', withJson({ method: 'PUT', body: JSON.stringify(input) }, token));
      return result.case;
    },
